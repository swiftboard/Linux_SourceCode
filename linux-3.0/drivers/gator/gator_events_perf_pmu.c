...
static DEFINE_PER_CPU(struct perf_event *[CNTMAX], pevent_static);
static DEFINE_PER_CPU(struct perf_event_attr *[CNTMAX], pevent_attr_static);
...
static int gator_events_perf_pmu_online(int** buffer)
{
	...
	for (cnt = 0; cnt < pmnc_counters; cnt++) {
		struct perf_event * ev = per_cpu(pevent_static, cpu)[cnt];
...
static void gator_events_perf_pmu_online_dispatch(int cpu)
{
	...
	for (cnt = 1; cnt < pmnc_counters; cnt++) {
		if (per_cpu(pevent_static, cpu)[cnt] != NULL || per_cpu(pevent_attr_static, cpu)[cnt] == 0)
			continue;

#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 1, 0)
		per_cpu(pevent_static, cpu)[cnt] = perf_event_create_kernel_counter(per_cpu(pevent_attr_static, cpu)[cnt], cpu, 0, dummy_handler);
#else
		per_cpu(pevent_static, cpu)[cnt] = perf_event_create_kernel_counter(per_cpu(pevent_attr_static, cpu)[cnt], cpu, 0, dummy_handler, 0);
#endif
		if (IS_ERR(per_cpu(pevent_static, cpu)[cnt])) {
			pr_debug("gator: unable to online a counter on cpu %d\n", cpu);
			per_cpu(pevent_static, cpu)[cnt] = NULL;
			continue;
		}

		if (per_cpu(pevent_static, cpu)[cnt]->state != PERF_EVENT_STATE_ACTIVE) {
			pr_debug("gator: inactive counter on cpu %d\n", cpu);
			perf_event_release_kernel(per_cpu(pevent_static, cpu)[cnt]);
			per_cpu(pevent_static, cpu)[cnt] = NULL;
			continue;
		}
	}
...
static void gator_events_perf_pmu_offline_dispatch(int cpu)
{
	...
	for (cnt = 0; cnt < pmnc_counters; cnt++) {
		...
		if (per_cpu(pevent_static, cpu)[cnt]) {
			pe = per_cpu(pevent_static, cpu)[cnt];
			per_cpu(pevent_static, cpu)[cnt] = NULL;
		}
...
static int gator_events_perf_pmu_start(void)
{
	...
	for_each_present_cpu(cpu) {
		for (cnt = 0; cnt < pmnc_counters; cnt++) {
			...
			per_cpu(perfPrevDelta, cpu)[cnt] = 0;
			per_cpu(pevent_attr_static, cpu)[cnt] = kmalloc(size, GFP_KERNEL);
			if (!per_cpu(pevent_attr_static, cpu)[cnt]) {
				gator_events_perf_pmu_stop();
				return -1;
			}

			memset(per_cpu(pevent_attr_static, cpu)[cnt], 0, size);
			per_cpu(pevent_attr_static, cpu)[cnt]->type = PERF_TYPE_RAW;
			per_cpu(pevent_attr_static, cpu)[cnt]->size = size;
			per_cpu(pevent_attr_static, cpu)[cnt]->config = pmnc_event[cnt];
			per_cpu(pevent_attr_static, cpu)[cnt]->sample_period = 0;
			per_cpu(pevent_attr_static, cpu)[cnt]->pinned = 1;

			// handle special case for ccnt
			if (cnt == ccnt) {
				per_cpu(pevent_attr_static, cpu)[cnt]->type = PERF_TYPE_HARDWARE;
				per_cpu(pevent_attr_static, cpu)[cnt]->config = PERF_COUNT_HW_CPU_CYCLES;
			}
		}
	}
...
static void gator_events_perf_pmu_stop(void)
{
	...
	for_each_present_cpu(cpu) {
		for (cnt = 0; cnt < pmnc_counters; cnt++) {
			if (per_cpu(pevent_attr_static, cpu)[cnt]) {
				kfree(per_cpu(pevent_attr_static, cpu)[cnt]);
				per_cpu(pevent_attr_static, cpu)[cnt] = NULL;
			}
		}
	}
