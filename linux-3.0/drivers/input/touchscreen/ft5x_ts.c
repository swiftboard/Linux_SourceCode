...
static void ft5x_report_multitouch(void)
{
...
#endif
	  printk("x=%d===Y=%d\n",event->x1,event->y1);
  //if((1==event->touch_point)&&((event->y1 > 490)&&(event->y1<510)))  //800*480
  if((1==event->touch_point)&&((event->y1 > 660)&&(event->y1<695)))
  {
  	key_tp = 1;
	 printk("===chenjie ------>KEY 1====event->x1=%d\n",event->x1);
  	if(event->x1 < 905)  //return 
  	{
  		key_val = 3;
	    input_report_key(data->input_dev, key_val, 1);
	    input_sync(data->input_dev);  
	//		#ifdef PRINT_POINT_INFO
			    printk("===KEY 1====event->x1=%d\n",event->x1);
	//		#endif	     
  	}
  	else if((event->x1 < 950)&&(event->x1>935))  //home 50~55
    	{
  		key_val = 2;
      		input_report_key(data->input_dev, key_val, 1);
      		input_sync(data->input_dev);     
	...
	//		#endif     		
    	...
    	else if((event->x1 < 1025)&&(event->x1>1015))  //menu
  	{
  		key_val = 1;
      		input_report_key(data->input_dev, key_val, 1);
      		input_sync(data->input_dev);     
  ...
  #if 0
  	else if ((event->y1 < 135)&&(event->y1>125))  //SERH 129
	{
  		key_val = 4;
                 input_report_key(data->input_dev, key_val, 1);
                input_sync(data->input_dev);     
