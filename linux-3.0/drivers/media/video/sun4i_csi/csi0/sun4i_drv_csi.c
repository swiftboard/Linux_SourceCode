...
extern int camera_detect(char * cam_name, int *i2c_addr, int isBack);
static int fetch_config(struct csi_dev *dev)
{
	...
	if(dev->dev_qty > 0)
	{
		...
		if((dev->ccm_cfg[0]->i2c_addr == 0xff) && (ret == 0))	//when insmod without parm
		{
			...
			camera_detect(dev->ccm_cfg[0]->ccm, &(dev->ccm_cfg[0]->i2c_addr), 0);
		}
	...
	if(dev->dev_qty > 1)
	{
		...
		if((dev->ccm_cfg[1]->i2c_addr == 0xff) && (ret == 0))	//when insmod without parm
		{
			...
			camera_detect(dev->ccm_cfg[1]->ccm, &(dev->ccm_cfg[1]->i2c_addr), 1);
		}
